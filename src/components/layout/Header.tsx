    ...
    >
      <div className="w-full max-w-(--max-width-content) mx-auto px-6 md:px-8 lg:px-12">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <Link
            href="/"
            className="text-text-primary font-semibold text-lg tracking-tight hover:opacity-80 transition-opacity"
            aria-label="Home"
          >
            SD
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-8" aria-label="Main navigation">
            {mainNavItems.map((item) => {
              const isActive =
                isHome && item.href.startsWith("#")
                  ? false // Handle with scroll detection
                  : pathname === item.href;
            ...
            <div className="flex items-center gap-4" aria-label="Social links">
              {socialNavItems.map((item) => (
                ...
                >
                  {item.label === "GitHub" && <GithubIcon />}
                  {item.label === "LinkedIn" && <LinkedInIcon />}
                  {item.label === "Email" && <EmailIcon />}
                </a>
              ))}
            </div>
          </nav>
    ...
    <div className="md:hidden">
      ...
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className="absolute top-16 left-0 right-0 bg-surface border-b border-border p-6 space-y-4"
        >
          {mainNavItems.map((item) => (
            <a
              key={item.href}
